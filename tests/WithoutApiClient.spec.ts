...
test.describe('Login tests', async () => {
  ...
  test('TL-12-1 Successful authorization,order creation and search', async ({request}) => {
    ...
    const responseSearchOrder = await request.get(`https://backend.tallinn-learning.ee/orders/${orderId}`, {
      headers:{
        Authorization: " Bearer " + await responseLogin.text()
      }
    });
  ...
  test('TL-12-2 Authorization and order deletion by ID', async ({ request }) => {
    ...
    const responseDeleteOrder = await request.delete(`https://backend.tallinn-learning.ee/orders/${orderId}`, {
      headers: {
        Authorization: "Bearer " + await responseLogin.text(),
      },
    });
    ...
    const responseSearchOrderAfterDeletion = await request.get(`https://backend.tallinn-learning.ee/orders/${orderId}`, {
      headers: {
        Authorization: "Bearer " + await responseLogin.text(),
      },
    });
